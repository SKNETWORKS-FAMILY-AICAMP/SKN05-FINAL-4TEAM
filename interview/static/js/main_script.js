...
//     return document.getElementById('csrd_token').ariaValueMax;
// }
// function getCSRFToken() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (realBtn ) {
        realBtn .addEventListener('click', async function () {
            ...
            try {
                // 1. 이력서 확인
                const resumeResponse = await fetch(`/api/check-resume?user_id=${userId}`);
                ...
                const checkInterval = setInterval(async () => {
                    const questionsResponse = await fetch(`/api/check-questions?user_id=${userId}`);
